...
Jev is a unique model that does not generate texts. A conventional LLM generates its verdict token by token. On the other hand, Jev produces only structured outputs from fixed set of options with decision probabilities. TypeSafe calls this a _System One_ model because (YUki: Add some short description here.). The architecture allows Jev to return the output in much lower latency, while aborting an ability to produce general text. See [their launch post] for learning more.
